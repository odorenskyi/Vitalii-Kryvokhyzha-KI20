...
void task10_1()
{
    ...
    if(!out_File.is_open())
    {
        cout << "ѕомилка в≥дкритт€ файлу!";
    }
    else{
        cout << "”сп≥шне в≥дкритт€ файлу";
        ...
        if(!in_File.is_open())
        {
            ...
            if(pos != -1)
            {
                out_File << "ѕомилок в текст≥ не маЇ" << "\n";
            }
            else{
                out_File << "ƒопущен≥ помилки в текст≥" << "\n";
            }
        }
    }
...
void task10_3()
{
    ...
    else
    {
        ...
        out_File << "„исло b у дв≥йковому код≥ = " << bitset<32>(b) << endl;
    }
    out_File.close();
}
